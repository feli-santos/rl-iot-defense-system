...
class AdversarialEnvConfig:
    ...
    Attributes:
        ...
        impact_penalty: Penalty applied during the agent's final action at the
            IMPACT stage. Should vastly exceed penalty_block_benign so that a
            system breach is always the worst possible outcome.
        defense_success_bonus: Bonus for successful defense (also used as
            partial mitigation bonus when BLOCK/ISOLATE is chosen at IMPACT).
...
class AdversarialIoTEnv(gym.Env):
    ...
    def step(
        ...
        # 0) Final action at IMPACT stage (one-step mitigation window)
        # When the attack has already reached IMPACT, allow the agent one final
        # defensive action before the episode terminates. The full impact_penalty
        # is always applied; choosing BLOCK or ISOLATE earns a partial mitigation
        # bonus, reducing the net loss slightly and providing a clear learning signal.
    ...
    def _check_terminated(self) -> bool:
        ...
        Note: IMPACT stage termination is handled directly in step() via an
        early-return guard that grants the agent one final action at that stage
        before the episode ends. This method is retained for future extensions
        (e.g., external stop signals) and currently always returns False.
        """
        return False
