...
class EpisodeGenerator:
    ...
    def __init__(
        ...
        self._stage_distribution = self._apply_laplace_smoothing(stage_distribution)
    ...
    def _apply_laplace_smoothing(
        ...
        if distribution is None:
            ...
            return dict.fromkeys(range(self._config.num_stages), 1.0 / self._config.num_stages)
        ...
        for stage_id in range(self._config.num_stages):
            ...
            if distribution.get(stage_id, 0) == 0:
                logger.warning(
                    f"Stage {stage_id} has no samples in dataset, using Laplace smoothing"
                )
    ...
    def generate_episode(self) -> list[int]:
        ...
        if self._rng.random() < self._config.benign_start_prob:
            ...
            attack_probs = np.array(
                [self._stage_distribution.get(i, 0.25) for i in range(1, self._config.num_stages)]
            )
    ...
    def _enforce_min_coverage(
        ...
        stage_coverage = dict.fromkeys(range(self._config.num_stages), 0)
        ...
        while needs_more and attempts < max_attempts:
            ...
            helps_with = [s for s in needs_more if s in unique_stages]
    ...
    def to_numpy(
        ...
        """
        sequences, targets = self.to_training_sequences(episodes, sequence_length)
