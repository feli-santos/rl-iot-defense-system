...
class TrainingManager:
    ...
    def __init__(self, algorithm: BaseAlgorithm, experiment_name: str, save_path: Path) -> None:
        """
        ...
        self.run_id = (
            f"{experiment_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:6]}"
        )
    ...
    def start_run(self, run_name: Optional[str] = None, nested: bool = False) -> None:
        ...
        try:
            ...
            self.current_run = mlflow.start_run(run_name=run_name or self.run_id, nested=nested)
        ...
        try:
            ...
            for key, value in params.items():
                ...
                if key not in existing_params:
                    ...
                    logger.warning(
                        f"Parameter conflict: {key}={existing_params[key]} vs {str_value}, logging as {new_key}"
                    )
    ...
    def train_algorithm(
        ...
        try:
            ...
            if self.current_run and mlflow_callback.episode_rewards:
                final_metrics = {
                    ...
                    "summary/reward_improvement": (
                        float(
                            np.mean(mlflow_callback.episode_rewards[-10:])
                            - np.mean(mlflow_callback.episode_rewards[:10])
                        )
                        if len(mlflow_callback.episode_rewards) >= 20
                        else 0.0
                    ),
                    "summary/average_fps": (
                        float(np.mean(mlflow_callback.fps_values))
                        if mlflow_callback.fps_values
                        else 0.0
                    ),
                }
            ...
            training_time = (
                (datetime.now() - start_time).total_seconds() if "start_time" in locals() else 0
            )
    ...
    def evaluate_algorithm(
        ...
        try:
            ...
            # Handle both VecEnv and regular env — both use `env` directly.
            eval_env = env  # noqa: SIM108
            ...
            results = {
                ...
                "success_rate": (
                    float(len([r for r in episode_rewards if r > 0]) / len(episode_rewards))
                    if episode_rewards
                    else 0.0
                ),
            }
