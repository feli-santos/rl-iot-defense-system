...
class RealizationEngine:
    ...
    def from_split_manifest(
        ...
        *,
        ...
        if rel_path not in manifest.get("outputs", {}):
            ...
            raise KeyError(
                f"split '{split_name}' not present in {manifest_path}. Available: {available}"
            )

        split_indices = np.load(manifest_dir / rel_path)
        allowed: set[int] = {int(i) for i in split_indices.tolist()}
    ...
    def _restrict_to(self, allowed_indices: Iterable[int]) -> None:
        ...
        allowed_set: set[int] = (
            allowed_indices
            if isinstance(allowed_indices, set)
            else {int(i) for i in allowed_indices}
        )
        if not allowed_set:
    ...
    def _load_artifacts(self) -> None:
        ...
        # Load features
        self._features: np.ndarray = np.load(self._data_path / "features.npy").astype(np.float32)
        ...
        if raw_features_path.exists():
            self._raw_features: Optional[np.ndarray] = np.load(raw_features_path).astype(np.float32)
        else:
            self._raw_features = None

        # Load state indices
        with open(self._data_path / "state_indices.json") as f:
            raw_indices = json.load(f)

        # Convert string keys to int
        self._state_indices: dict[int, list[int]] = {int(k): v for k, v in raw_indices.items()}

        # Load scaler
        self._scaler: StandardScaler = joblib.load(self._data_path / "scaler.joblib")

        # Cache counts for weighted sampling
        self._stage_counts = {
            stage_id: len(indices) for stage_id, indices in self._state_indices.items()
        }
        self._total_samples = sum(self._stage_counts.values())
