...
class CICIoTDataset(Dataset):
    ...
    def __init__(self, sequences: np.ndarray, labels: np.ndarray) -> None:
        ...
        logger.info(
            f"Dataset initialized: {len(self.sequences)} samples, "
...
class CICIoTDataLoader:
    ...
    def _load_artifacts(self) -> None:
        ...
        try:
            ...
            raise FileNotFoundError(f"Preprocessing artifacts not found or corrupted: {e}") from e
    ...
    def load_lstm_data(self) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        try:
            ...
            sequences = {"train": train_sequences, "val": val_sequences, "test": test_sequences}

            targets = {"train": train_targets, "val": val_targets, "test": test_targets}

            logger.info(
                f"LSTM data loaded - Train: {len(train_sequences)}, "
            ...
            raise FileNotFoundError(f"LSTM sequences not found in {self.data_path}") from e
        except Exception as e:
    ...
    def load_rl_states(self) -> np.ndarray:
        ...
        try:
            ...
            raise FileNotFoundError(f"Test sequences not found for RL states: {e}") from e
        except Exception as e:
