...
% Latin Modern ships no bold-extended small-caps face (T1/lmr/bx/sc); declare a
% silent substitution to the bold-extended upright shape so bold small-caps
% (e.g. \textbf{\textsc{...}}) renders bold without emitting a font-shape warning.
...
\usepackage[alf,bibjustif,abnt-etal-cite=2,abnt-etal-list=0,abnt-etal-text=emph]{abntex2cite}	% Cita\c{c}\~{o}es padr\~{a}o ABNT (bibjustif = lista de refer\^{e}ncias justificada)
% ---
...
% ---
% Float-placement tuning: allow more floats per page so LaTeX does not
% strand a single figure on an otherwise empty page. Placement is still
% left to LaTeX (floats keep [!htbp]/[!htb]; no forced [H]).
% ---
\renewcommand{\topfraction}{0.9}        % up to 90% of a page top may be floats
\renewcommand{\bottomfraction}{0.9}     % up to 90% of a page bottom may be floats
\renewcommand{\textfraction}{0.1}       % require only 10% text on a mixed page
\renewcommand{\floatpagefraction}{0.75} % a float page must be >=75% full
\renewcommand{\dbltopfraction}{0.9}     % same, for full-width (starred) floats
\renewcommand{\dblfloatpagefraction}{0.75}
\setcounter{topnumber}{3}               % up to 3 floats at the top of a page
\setcounter{bottomnumber}{2}            % up to 2 floats at the bottom
\setcounter{totalnumber}{4}             % up to 4 floats on a single page
...
% Override abnTeX2/babel Portuguese caption names at \begin{document},
% after babel has set the brazilian defaults.
\AtBeginDocument{
...
\hypersetup{
        %pagebackref=true,
		pdftitle={A Deep Reinforcement Learning Framework for Autonomous Cybersecurity Defense in IoT Networks},
...
%		linkbordercolor={1 1 1},	% set to white
		plainpages=false,			% distinct PDF page anchors across \pagenumbering resets (roman front matter -> arabic main matter)
		hypertexnames=false,		% derive page anchors from the physical page counter, eliminating duplicate name{page.N} destinations
		bookmarksdepth=4
}
\makeatother
% ---
...
% ---
\titulo{A Deep Reinforcement Learning Framework for Autonomous Cybersecurity Defense in IoT Networks\\[12pt]{\large Um Arcabou\c{c}o de Aprendizado por Refor\c{c}o Profundo para Defesa Cibern\'{e}tica Aut\^{o}noma em Redes IoT}}
...
% abnTeX2 typesets the folha-de-rosto preambulo in a right-anchored
% minipage of width .5\textwidth preceded by \hspace{.45\textwidth}. At this
% class's 16cm text block the minipage's right edge crosses the margin,
% producing a 14.01pt Overfull \hbox at the \imprimirfolhaderosto call (the
% "lines 54--54" warning). No hyphenation/ragged/emergencystretch tweak helps
% because a minipage takes its full declared width regardless of inner
% content; only narrowing the box removes the protrusion. Patch the width
% down to .45\textwidth (symmetric with the .45 lead hspace, verified to
% eliminate the overfull while leaving the text-block visually unchanged).
\usepackage{etoolbox}
\makeatletter
\patchcmd{\folhaderostocontent}%
  {\begin{minipage}{.5\textwidth}}%
  {\begin{minipage}{.45\textwidth}}%
  {}{\PackageWarning{preamble}{folha-de-rosto minipage patch failed}}
\makeatother
% ---
