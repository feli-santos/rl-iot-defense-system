...
\toprule
Stage & Pass & Pass+finding & Pass no stretch & Fail+finding & Headline gate \\
\midrule
Detector (\texttt{G4}) & 4 & 1 & --- & --- & G4.4: OOD recall asymmetry ($\Delta = 0.998$) \\
Blue-team (\texttt{G5}) & 6 & 1 & --- & --- & G5.4: prevention-rate primary KPI; best-algo prevention $0.602$ \\
...
# expect: VERDICT PASS; 0 FAIL / 2 KNOWN-DIVERGENCE
\end{verbatim}
...
make blue-team-sweep BLUE_TEAM_TIMESTEPS=250000
make benchmark               # held-out benchmark + F5/F6/F7/F8
make ablation                # ablations + F9/F10/F12/F15
...
For DQN-specific off-policy hyperparameters, the values used were: replay buffer $50{,}000$, learning starts $1000$, target update interval $1000$, exploration fraction $0.10$, initial/final exploration $1.0/0.05$, and max gradient norm $0.5$. The complete machine-readable record is at \path{docs/results/05_blue_team/T1_hparams.json}.

The ablation and OOD-robustness evaluations re-use the PPO hyperparameters above unchanged; only the environment contract and reward coefficients are perturbed.
