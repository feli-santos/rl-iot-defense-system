...
\subsection{Per-Stage Manifest Pattern}\label{app:manifest_pattern}
...
2 (Red-team LSTM) & \texttt{02\_red\_team/} & \texttt{F1\_learning\_curves.png}, \texttt{F2\_transition\_matrix\_comparison.png}, \texttt{F1/F2\_summary.json}, \texttt{manifest.json}, \texttt{RESULTS.md} \\
...
Blue-team DRL (\texttt{G5}) & 6 & 1 & --- & --- & G5.4 (reward-hacking, mit-rate $0.263$) \\
Benchmark (\texttt{G6}) & 4 & 1 & --- & 1 & G6.2 (audit-AF2 reframe, 82\,\% of oracle ceiling) \\
Ablation/OOD (\texttt{G7}) & 7 & --- & 1 & 2 & G7.2 (F9 \texttt{impact\_is\_terminal}=False), G7.9 (D7.9.1 OOD) \\
\hline
\end{tabular}
\end{table}
...
\begin{description}
    ...
    \item[\textsc{known-divergence}] The recorded SHA does not match on-disk \emph{and} the divergence is documented and audited. The harness ships with a \texttt{\_KNOWN\_DIVERGENCES} table that explicitly enumerates every such case with its finding-id and explanatory note. The two pre-registered divergences in this dissertation are the dataset-preparation and red-team-training stage manifests' recorded splits-manifest SHA (\texttt{82aa1214\ldots}, the pre-leakage-fix manifest) versus the on-disk post-fix SHA (\texttt{1e99d596\ldots}); both are documented in \texttt{docs/results/02\_red\_team/RESULTS.md} \S5.3.
...
The thesis PDF is compiled via Docker (no host-side \LaTeX{} installation required):
...
\begin{table}[h!]
\centering
\caption{Blue-team training per-algorithm hyperparameters. Values are frozen at PLAN \S8 D5.4 and used across all ten seeds and 250{,}000 environment steps per run. N/A indicates the hyperparameter does not apply to the algorithm.}
\label{tab:hparams}
\small
