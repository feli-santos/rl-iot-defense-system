...
% --- Lane 2: Markov Attacker ---
\node[stageBox, fill=colOffline, right=of dataset] (redteam) {
  \textbf{Markov Attacker}\\[3pt]
  Fixed $5\times5$ first-order\\
  kill-chain transition\\
  matrix\\[3pt]
  Upper-triangular;\\
  absorbing \textsc{impact}\\[3pt]
  Finite intrusion\\
  budget ($B=40$)
};
...
% Stage I → Stage II (Markov attacker to env)
