...
reads each cell's ``eval.jsonl`` (last 10% of eval episodes as final
reward), then plots mean ± 95% bootstrap CI per scale value.
"""
...
def build_summary(
    ...
    scales = sorted({r["scale"] for r in manifest["runs"]})
...
def plot(summary: Dict[str, Any], out_dir: str) -> Path:
    ...
    ax.set_xlabel("Action Cost Scale", fontsize=11)
    ax.set_ylabel("Mean Episode Reward", fontsize=11)
    ax.set_title(
        "Action-Cost Scale Sensitivity (PPO, 3 seeds × 30 ep, last 10%)",
        fontsize=10,
    )
    ...
    out_path = Path(out_dir) / "FA_action_cost_sweep.png"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    logger.info("FA_action_cost_sweep: wrote %s", out_path)
    return out_path
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    json_path = out_dir / "FA_action_cost_sweep_summary.json"
