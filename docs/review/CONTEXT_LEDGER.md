...
### Caveat Ledger (C1–C10) — implementation hazards for the new mechanics
...
| **C6** | **(RESOLVED)** "Which 29 features & why" — the 46→29 selection is deterministic and leakage-safe (`_apply_feature_selection`, `src/utils/dataset_processor.py:281-342`): 3 stages — zero-variance drop, low-variance drop (`variance_threshold=0.01`), high-correlation drop (`correlation_threshold=0.95`), all fit on the **train split only**. The canonical 29 ordered names live in the gitignored `data/processed/ciciot2023/metadata.json` (`feature_columns` + `feature_selection_info`); the only committed enumeration was prose in `docs/dataset_card.md:91-125`. **FIX:** added a committed, machine-readable, hash-pinned artifact `docs/results/dataset/feature_provenance.json` — lists the 29 columns in canonical order, the thematic grouping, the 17 dropped (4 zero-variance / 7 low-variance / 6 high-correlation), the thresholds, the train-only scaler, the obs-dim note (5×29×2=290), and pins the `metadata.json` SHA-256 (`a9e68b68…`). Validated to match `metadata.json` byte-for-byte. | RESOLVED — cite `docs/results/dataset/feature_provenance.json` in Phase F threats-to-validity (feature provenance). |
...
| **C10** | **(RESOLVED)** **(PROVENANCE GAP, not a functional bug)** Benchmark `eval_manifest.json` recorded `eval_env.attacker_budget: None` (and omitted `impact_is_terminal`/`evasion_prob`) even when `--attacker-budget 40` was passed — the top-level `eval_env` metadata block was hand-rolled from a bare `_eval_env_spec()` (no budget arg) listing only 7 fields. The budget DID apply (verified empirically: budget=40 → `compromise_rate` 0.357–0.840 across policies vs 1.000 for every policy in the budget=None control cell). **NB: the blue-team `run_manifest.json` was NEVER affected** — it serialises `asdict(cfg.env)`/`asdict(cfg.eval_env)` on the merged specs and already recorded `attacker_budget=40, impact_is_terminal=False, evasion_prob=0.0` faithfully (confirmed on all three live seed_0 manifests). **FIX:** `run_test_eval.py` now builds the manifest `eval_env` block via `dataclasses.asdict(_eval_env_spec(args.attacker_budget))`, so the full 29-field spec (incl. the `attacker_budget`/`evasion_prob`/`impact_is_terminal` trio) is recorded faithfully; control cell records `attacker_budget=None`. Regression guard: `tests/test_run_test_eval_manifest.py` (4 tests). Provenance is now self-contained in the manifest; the CLI-flag-not-manifest workaround is no longer required. | RESOLVED — re-run `make benchmark-eval` to regenerate the two stale `eval_manifest.json` (produced by the old code) with faithful provenance. |
