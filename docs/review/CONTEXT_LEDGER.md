...
| **C10** | **(PROVENANCE GAP, not a functional bug)** Benchmark `eval_manifest.json` records `eval_env.attacker_budget: None` (and `impact_is_terminal: None`) even when `--attacker-budget 40` is passed — the top-level `eval_env` metadata block is unpopulated for these fields. The budget DID apply (verified empirically: budget=40 → `compromise_rate` 0.357–0.840 across policies vs 1.000 for every policy in the budget=None control cell). Provenance therefore relies on the **CLI flag + the per-episode eval JSONL outcomes**, NOT the manifest block. Same class as the existing AGENTS.md note about the eval manifest not recording `impact_is_terminal`. | Document in Phase F threats-to-validity / reproducibility; optionally populate the field in a later cleanup commit. |
