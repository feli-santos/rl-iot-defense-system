...
- `docs/results/ablation/ood_robustness.png` — 4-class
...
- `docs/results/ablation/ood_robustness.json` — per
...
- `docs/results/ablation/ood_robustness.caption.md` — one-paragraph
  thesis caption.
- `docs/results/ablation/ood_robustness_manifest.json` — SHA-256 hash
...
- `docs/results/ablation/reward_ablation.png` — per-component
...
- `docs/results/ablation/reward_ablation.json` — per-cell
  ...
  CI separation flag).
- `docs/results/ablation/reward_ablation.caption.md`.
- `docs/results/ablation/reward_ablation_manifest.json`.
...
Emits `aggressiveness.png` — x-axis `p_defender_deescalation`,
...
Emits `pareto.png` + `pareto_frontier.json` (frontier points
list + dominance count) + `pareto_frontier.caption.md` + `pareto_frontier_manifest.json`.
...
`ablation_acceptance.json` records `passes:false` permanently in such
...
|---|---|:---:|---|---|
| **F9** | `docs/results/ablation/reward_ablation.png` | 2 | Phase 7 | Per-component test-reward effect at 0.5× / 1× / 2× of Phase-3 defaults; Phase-6 oracle ceiling and deployable best as reference lines. |
| **F10** | `docs/results/ablation/aggressiveness.png` | 2 | Phase 7 | PPO and oracle-rule mean test reward as a function of `p_defender_deescalation`; bands = 95 % bootstrap CI. |
| **F12** | `docs/results/ablation/pareto.png` | 2 | Phase 7 | Security gain (1 − compromise_rate) vs. availability cost (BLOCK+ISOLATE share); Pareto frontier highlighted. |
| **F15** | `docs/results/ablation/ood_robustness.png` | **1** | Phase 7 (audit AF1) | 4-class × 8-policy grouped horizontal bar chart of mean test reward under OOD-class-restricted realisations. |
...
|:-:|---|---|:-:|:-:|
| C1 | `docs(phase-6,§6.4): reframe rec-action floor as oracle upper bound (audit AF2)` | `docs/results/benchmark/RESULTS.md` only | 20 m | 0 |
| C2 | `docs(phase-7): audit & PLAN — F9 + F10 + F12 + F15` (this commit) | `docs/results/ablation/PLAN.md`, `docs/thesis_results_map.md` (add F15 row, promote F9/F10/F12 to Tier 2 confirmed) | 1.5 h | 0 |
...
| C9 | `docs(phase-7): close — RESULTS + CHANGELOG + G7 scoreboard` | `docs/results/ablation/RESULTS.md`, `CHANGELOG.md`, `docs/results/ablation/ablation_acceptance.json` | 2 h | 0 |
...
| **D7.10** | Phase-7 figures live under `docs/results/ablation/` (singular `07_ablation`, not `07_ablations`). | Consistent with `02_red_team`, `03_env`, `04_detector`, `05_blue_team`, `06_benchmark` (each is a single noun-phrase). |
...
- **D7.9.1** (locked 2026-05-01 — fully ACTIVATED) — R7.2
  ...
  `ablation_acceptance.json#gates[8].threshold`. **Thesis claim
