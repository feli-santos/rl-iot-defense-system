...
> `F1_summary.json`, `manifest.json`, `attack_sequence_generator.pth`)
...
Source of record: `F1_summary.json` next to this file
(SHA `1c7d26eabdbb...` per `manifest.json#outputs`).

## 2 — Gate scoreboard

The Phase-2 numerical record is `F1_summary.json::gates_values`:
...
The G1_balanced_val_loss_for_reference field (`F1_summary.json#gates_values.G1_balanced_val_loss_for_reference = 0.916`)
...
|---|---|---|
| **F1** | `F1_learning_curves.png` + `F1_summary.json` | Training/validation cross-entropy curve over the 15 trained epochs + macro-F1 monitoring overlay; hand-locked at the early-stop epoch. |
| **F2** | `F2_transition_matrix_comparison.png` | 5×5 transition-matrix grid: LSTM-implied (right) vs. ground-truth-from-corpus (left); the visual G3+G4 evidence. |
| **Captions** | `F1_learning_curves.caption.md`, `F2_transition_matrix_comparison.caption.md` | One-paragraph thesis-paper captions per figure. |
...
- The `val_macro_f1` metric is logged per epoch and rendered on
  `F1_learning_curves.png` (green curve) but is **not** consulted
  for early-stop — it is a **monitoring signal**, not a
  selection criterion.
- F1_summary.json records `best_epoch = 1`, `best_val_loss = 0.854`,
...
`docs/results/02_red_team/manifest.json` records the Phase-1 splits
...
`docs/results/01_dataset/manifest.json` is SHA `c8574094...`
(post-`3cd2fb9`).
...
- **(a)** Re-run the LSTM training with `seed = 42` against the
  post-`3cd2fb9` Phase-1 splits manifest, regenerate `F1_summary.json`
...
`docs/results/01_dataset/manifest.json` (SHA `c8574094...`) for the
...
under `docs/results/02_red_team/` (PLAN.md, F1/F2 PNGs + captions,
F1_summary.json, manifest.json) are sufficient to verify the gate
