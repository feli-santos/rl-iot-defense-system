...
across 5 seeds), aligned with IoTWarden Fig. 5.
...
IoTWarden recommended policy is roughly LOG ≫ OBSERVE > BLOCK >
...
late-checkpoint argmax in every panel matches the IoTWarden
recommended action at that stage (LOG@RECON, BLOCK@MANEUVER, …),
demonstrating that the agent learned **stage-action proportionality**
...
Caption credit: aligned with IoTWarden Fig. 5. Per-stage and per-bin
counts in `F4_summary.json`; SHA-256 inputs/outputs in
`F4_manifest.json`.
