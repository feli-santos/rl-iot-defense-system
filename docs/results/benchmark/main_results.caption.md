...
**Files:**
- `main_results.json` — machine-readable per-policy metrics + bootstrap CIs.
- `main_results.md`   — Markdown table for the thesis chapter.
- `main_results.csv`  — flat per-row metrics.
- `main_results_table.png`    — rendered table (best row highlighted, divider
                      between trained-RL and non-RL baselines).
- `main_results_manifest.json` — input/output SHA-256 chain + git SHA.
