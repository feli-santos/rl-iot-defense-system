...
>   not the underlying classifier; see benchmark_acceptance.json status
...
> `sweep_manifest.json` are SHA-256 hash-pinned in `latency_profile_manifest.json`;
> the platform fingerprint (`platform.platform()`, `platform.processor()`,
> Python version) is recorded in `latency_profile.json` so absolute
...
**Files:**
- `overhead.png` — two-panel figure (left CDF, right training-time bar).
- `latency_profile.json` — per-policy {p50, p95, p99, mean} latency in ms +
                      per-algo training seconds + platform fingerprint
                      + per-policy `g64_pass` boolean.
- `latency_profile_manifest.json` — input/output SHA-256 chain + git SHA.
