...
**Files:**
- `baselines.png` — horizontal bar chart with bootstrap-CI whiskers
                       and oracle recommended-action ceiling annotated.
- `reward_ranking.json`  — sorted-desc per-policy {mean, ci_low, ci_high} +
                       per-RL-policy G6.5 pass/fail and overlap list.
- `reward_ranking_manifest.json` — input/output SHA-256 chain + git SHA.
