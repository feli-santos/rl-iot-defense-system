...
Two-panel diagnostic of the **best-performing algo** (PPO,
...
No per-stage action share exceeds 70 % at the late
checkpoint:
...
(via `end_outcome`) is the G5.4 narrative.

Per-stage and per-bin counts in `action_distribution.json`; SHA-256
inputs/outputs in `action_distribution_manifest.json`.
