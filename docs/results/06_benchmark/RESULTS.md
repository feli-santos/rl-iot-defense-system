...
>
> **Canonical data source:** `F5_summary.json` (n_seeds=10, n_episodes=300,
...
| **F7** | `F7_overhead.png` + `F7_summary.json` | Two-panel: left = inference-latency CDF (log-x) per policy; right = training wallclock per algo (sum over seeds). |
| **F8** | `F8_baselines.png` + `F8_summary.json` | Horizontal bar chart of mean reward with 95 % bootstrap CIs, sorted descending; oracle ceiling reference line. |
| Captions | `F5_caption.md` … `F8_caption.md` | One-paragraph thesis captions per figure. |
| Manifests | `F5_manifest.json` … `F8_manifest.json` | SHA-256 hash chain over input JSONLs + upstream eval manifest + git SHA at production time. |
| Scoreboard | `G6_scoreboard.json` | Per-gate threshold + value + status (canonical; regenerated 2025-06-04). |
...
policies. See `docs/results/07_ablation/RESULTS.md` for findings.
