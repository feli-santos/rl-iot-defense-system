...
> Phases 3–5 (`docs/results/05_blue_team/PLAN.md`). The PLAN is committed
...
Phase 5 already proved each of the trained trio beats the IoTWarden
recommended-action floor (~+50) by ~25× (PPO +1350.7, A2C +1325.6,
DQN +1300.1, mean over 5 seeds × last-10 % × 30 episodes). Phase 6
turns those frozen checkpoints into the four publishable comparisons
...
| `scripts/benchmark/build_summary_table.py` (**F5**) | reads all eval JSONLs + latency JSONLs → `docs/results/06_benchmark/F5_summary.{json,md,csv,png}` + `F5_manifest.json`. Columns: `mean_reward`, `mean_mttc`, `compromise_rate`, `mitigated_impact_rate`, `mean_episode_length`, `mean_inference_latency_ms`, `p95_inference_latency_ms`. | ~150 |
| `scripts/benchmark/plot_stage_action_cm.py` (**F6**) | one 5×5 row-normalised heatmap per algo from `action_counts_by_stage` aggregated across 5 seeds × 30 episodes; → `F6_stage_action_cm.png` + `F6_summary.json` + `F6_manifest.json`. | ~120 |
| `scripts/benchmark/plot_overhead.py` (**F7**) | left panel: per-step inference latency CDF (one curve per policy) from `latency.jsonl`; right panel: training-time bar from `runs/phase5/sweep_manifest.json` (sum-over-seeds wallclock per algo). → `F7_overhead.png` + `F7_summary.json` + `F7_manifest.json`. | ~140 |
| `scripts/benchmark/plot_baselines.py` (**F8**) | bar of `mean_reward` across all 8 policies with 95 % bootstrap CI; horizontal line at `recommended_action` floor for visual reference. → `F8_baselines.png` + `F8_summary.json` + `F8_manifest.json`. | ~110 |
...
|---|---|---|---|
| C1 | `docs(phase-6): audit & PLAN — F5+F6+F7+F8 RL benchmark` | `docs/results/06_benchmark/PLAN.md` | this commit |
...
| C8 | `docs(phase-6): close — RESULTS + CHANGELOG + G6 scoreboard` | `RESULTS.md`, `CHANGELOG.md`, `G6_scoreboard.json` | ~30 min |
...
| **D6.2** | Eval split for F5/F6/F7/F8 is **`test_balanced`** (the held-out split, untouched by training-time decisions). Phase-5 val-split numbers stay reported as model-selection metrics. | Honest use of the held-out split; correct for the thesis's headline numbers. Both splits' hashes pinned in `F5_manifest.json`. |
| **D6.3** | **30 deterministic eval episodes × 5 seeds** per (algo or non-deterministic baseline). Deterministic baselines (always-OBSERVE, always-BLOCK, recommended-action, RF-acting) get 1 seed × 150 episodes (same total n). | Matches Phase-5 `n_eval_episodes=30`; n=150 episodes/policy yields tight bootstrap CIs for G6.5. |
...
| **D6.9** | Manifests use the SHA-256 hash chain convention from Phase 1 / 4 / 5. Each Phase-6 figure ships `<F>_manifest.json` containing: input JSONL SHAs, RF model SHA, scaler SHA (if used), splits-manifest SHA, git SHA at production time. | Reproducibility contract; matches existing `docs/results/05_blue_team/{F3,F4}_manifest.json` schema. |
| **D6.10** | The "best algo" sticker shown in F5 / F8 is selected by **mean reward** on `test_balanced` (tie-breaker: lower p95 latency). | Consistent with Phase-5's `best_algo` field in `G5_scoreboard.json`. PPO is expected to stay the best by Phase-5 evidence, but the choice is data-driven. |

### Follow-up D-decisions (post-PLAN-lock)

| ID | Decision | Date | Rationale |
|---|---|---|---|
| **D6.2.1** | **G6.2 threshold revised on first contact with `test_balanced` evidence.** Original threshold ("trained-RL `mean_reward` ≥ 5× recommended-action") was anchored on Phase-5's val-split estimate that the recommended-action floor was ~+50. The C3 sweep on the held-out `test_balanced` split revealed that the recommended-action floor is **+1624**, *higher* than every trained RL algo (PPO +1313, A2C +1297, DQN +1336). Phase 5's "25× over baseline" claim was therefore a **selection-bias artefact of the val-split rollouts**: the trained agents converged on a de-escalation-farming strategy (G5.4 PASS-WITH-FINDING) that scored well on val but does not generalise to the held-out split. **Revised G6.2 threshold:** trained-RL `mean_reward` strictly exceeds recommended-action `mean_reward` (i.e., a 1× floor). The original 5× threshold is **declared FAIL on `test_balanced`** and the failure is the headline finding of Phase 6, not a gate-tuning issue. | 2026-04-30 | This is exactly the audit-first protocol that paid off in Phases 3, 4, and 5: gate fails → ask "is the gate or the implementation wrong?" → here, the implementation (Phase-5 training) is fine, the *gate's framing* mistook a val-split metric for a production claim. The honest move is to surface the finding, not relax the gate silently. Phase 7's reward-component ablation now has a sharply-defined target: "show why a hand-crafted rule (recommended-action) outperforms 250 K-step trained RL on the held-out split, and design a reward landscape that closes the gap." This recasts the thesis's empirical chapter from "RL beats baselines by 25×" to the more defensible "we identify a precise generalisation gap between Phase-3 reward-shaping and held-out performance, and motivate the Phase-7 ablation that closes it." See RESULTS.md §5–§6 for the full narrative. |
| **D6.8.1** | **G6.4 declared FAIL-WITH-FINDING for the RF-Acting baseline only.** Empirical p50 inference latency on `test_balanced` (Apple silicon CPU, single process): RL trio 0.07–0.10 ms ✓ (≥ 30× headroom on the 5 ms budget), rule baselines ≤ 0.002 ms ✓ (≥ 500× headroom on the 1 ms budget), **RF-Acting 13.976 ms ✗** (budget 3 ms; ~4.7× over). The cost is dominated by sklearn's per-call Python dispatch over the 100-tree ensemble, *not* by the underlying RF detector head (which already passed Phase-4 G4.5 ≤ 1 ms on per-flow inputs). Phase 6 records this as a property of the *supervised-wrapper-as-policy* construction (D6.5) rather than as a budget violation to be patched: RF-Acting trades inference cost for **higher reward than the RL trio** (+1508 vs. +1297..+1336), and any deployment would naturally batch the per-step inference or compile the forest with treelite/skl2onnx — out of scope for Phase 6. The remaining seven policies all pass G6.4 by ≥ 30× headroom; the headline RL latency claim is unaffected. | 2026-04-30 | Same audit-first protocol as D6.2.1: gate fails → ask "is the gate or the implementation wrong?" → here, the budget is fine for production-realistic deployments (batched / compiled), but the apples-to-apples per-call comparison in F7 deserves to surface the gap rather than relax it. The thesis story this enables: "RF-Acting wins reward but loses inference cost; RL wins inference cost but loses reward — the cross-quadrant trade-off that motivates the Phase-7 reward-component ablation as a way to *get both*." |
