...
| **G6.2** | F5 table populated for {dqn, ppo, a2c, random, always_observe, always_block, recommended_action, rf_acting} on every metric column | every cell non-NaN; trained-RL row `mean_reward` ≥ **5×** the `recommended_action` row's `mean_reward` |
