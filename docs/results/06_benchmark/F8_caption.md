...
> `test_balanced` with 95 % bootstrap CIs**, sorted descending. Trained-RL
> bars in blue; non-RL baselines in grey. The red dashed line marks
> the **Recommended-Action floor (+1624.4)** — the IoTWarden hand-
> crafted rule baseline. Every CI annotation is rendered next to its
> bar (mean and 95 % bootstrap interval bounds).
>
...
>
> - **The trained-RL cluster** sits ~+290 below the rule floor but
>   ~+780 above the random-policy bar — the agents *did* learn
...
- `F8_baselines.png` — horizontal bar chart with bootstrap-CI whiskers
                       and recommended-action floor annotated.
